...
{
    public class DescriptionUpdater<TContext> where TContext : DbContext
    {
        ...
        private void SetTableDescriptions(Type tableType)
        {
            ...
            var dbTableDescattr = tableType.GetCustomAttribute(typeof(DescriptionAttribute), false);
            string tableComment = ((DescriptionAttribute)dbTableDescattr).Description;
            ...
            {
                var dbDescAttr = prop.GetCustomAttribute(typeof(DescriptionAttribute), false);
                ...
                if (columnNameAttr != null)
                {
                    ...
                    SetDBDescription(tableName, string.IsNullOrEmpty(columnName) ? prop.Name : columnName,
                        ((DescriptionAttribute)dbDescAttr).Description);
                }

                else
                {
                    SetDBDescription(tableName, prop.Name, ((DescriptionAttribute)dbDescAttr).Description);
                }
            }
